...
ARTIFACT_ROOT = REPO_ROOT / "build" / "artifacts" / "hdlbits_gsim_runtime_remaining_test"
...
REMAINING_DUT_IDS = (
    "026,040,041,042,043,071,095,108,109,118,125,126,130,131,135,136,137,138,"
    "139,141,143,144,151,154,155,156,157,162"
)


def fail(message: str) -> int:
    print(f"[hdlbits-gsim-runtime-remaining] {message}", file=sys.stderr)
    return 1
...
def main() -> int:
    try:
        ...
        expect(
            result.returncode == 0,
            "remaining runtime harness failed\nstdout:\n"
            + (result.stdout or "")
            + "\nstderr:\n"
            + (result.stderr or ""),
        )
        ...
        expect(report_path.exists(), "missing remaining runtime report")
        report = json.loads(report_path.read_text(encoding="utf-8"))
        summary = report["summary"]
        expect(summary["total_duts"] == 28, f"expected 28 DUTs, got {summary['total_duts']}")
        expect(summary["success"] == 28, f"expected 28 successes, got {summary['success']}")
        ...
        for dut_id in REMAINING_DUT_IDS.split(","):
